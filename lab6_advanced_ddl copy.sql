...
--2.3
SELECT e.emp_name, p.project_name FROM employees e CROSS JOIN projects;

--part3--
--3.1
SELECT e.emp_name, d.dept_name, d.location FROM employees e INNER JOIN departments d ON e.dept_id = d.dept_id;
--3.2
SELECT e.emp_name, d.dept_name, location FROM employees e INNER JOIN departments d USING(dept_id);
--3.3
SELECT emp_name, dept_name, location FROM employees NATURAL JOIN departments;
--3.4
SELECT e.emp_name, d.dept_name, p.project_name FROM employees e
INNER JOIN departments d ON e.dept_id = d.dept_id
INNER JOIN projects p ON e.dept_id = p.dept_id;

--part4--
--4.1
SELECT e.emp_name, e.dept_id_id AS emp_dept, d.dept_id AS dept_dept, d.dept_name
FROM employees d LEFT JOIN departments d ON e.dept_id = d.dept_id;
--4.2
SELECT e.emp_name, e.dept_id_id AS emp_dept, d.dept_id AS dept_dept, d.dept_name
...
--4.4
SELECT d.dept_name, COUNT(e.emp_name) AS employee_count FROM departments d
...
--6.2
SELECT d.dept_name, p.project_name, p.budget FROM departments d 
FULL JOIN projects p on d.dept_id = p.dept_id;
--6.3
SELECT 
    CASE
        WHEN e.emp_id IS NULL THEN 'Department without employees'
        WHEN d.dept_id IS NULL THEN 'Employe without department'
        ELSE 'Matched'
...
--part8--
--8.1
SELECT d.dept_name, e.emp_name, e.salary, p.project_name, p.budget FROM departments d
LEFT JOIN employees e ON d.dept_id = e.dept_id
LEFT JOIN projects p ON d.dept_id = p.dept.emp_id
